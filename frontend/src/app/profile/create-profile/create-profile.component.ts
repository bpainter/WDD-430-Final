...
export class CreateProfileComponent implements OnInit {
  ...
  getProfileById(id) {
    ...
    this.profileService.getProfileByUsername(id, token).subscribe(profile => {
      this.isLoading = false
      this.post = {
        id: profile.profile._id,
        username: profile.profile.username,
        bio: profile.profile.bio,
        imagePath: 'backend/' + profile.profile.imagePath,
        creator: profile.profile.creator
      };
      this.profileId = profile.profile._id
      this.imagePreview = 'backend/' + profile.profile.imagePath
      this.form.setValue({
  ...
  checkUsername(uname) {
    const token = this.authService.getToken();
    this.profileService.getProfileByUsername(uname, token).subscribe(profile => {
      if (profile.profile && uname !== this.uname) {
        this.error = "Try another username, this one's taken.";
      } else {
        this.error = null;
      }
    })
